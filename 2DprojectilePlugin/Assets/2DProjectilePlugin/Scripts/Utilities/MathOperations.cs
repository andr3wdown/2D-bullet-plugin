...
namespace UpGamesWeapon2D
{
    namespace Math
    {
        public static class MathOperations
        {
            ...
            //generate a float array of random numbers
            ...
            public static Transform FindClosestTarget(Collider2D[] objArr, Transform searcher)
            {
                float distance = float.MaxValue;
                Transform target = null;
                ...
                for (int i = 0; i < objArr.Length; i++)
                {
                    if (Vector2.Distance(searcher.position, objArr[i].transform.position) < distance)
                    {
                        distance = Vector2.Distance(searcher.position, objArr[i].transform.position);
                        target = objArr[i].transform;
                    }
                }
            ...
            //rotate towards a target in 2d space
